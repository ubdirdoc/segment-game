...
public class MakeTextTemporallyAppears : MonoBehaviour {
	public Text m_textToMakeAppear;
	...
	IEnumerator BlinkCoroutine() {
		if (!m_isAlreadyBlinking) {
			...
			Color initialColor = m_textToMakeAppear.color;
			...
			for (int i = 0; i < m_nbOfBlink; i++) {
				...
				while (lerpValue < 1) {
					...
					m_textToMakeAppear.color = Color.Lerp(initialColor, m_colorToBlinkTo, lerpValue);
					yield return null;
				}
